...
| `deployment/bitrix/` | Reproducible delivery projection for the `ver.cy` Bitrix site: two deliberately separate information blocks for desired subject meta-models and interoperability references, the `/models/` view, dynamic TODO-card routing and sitemap integration. Bitrix remains a projection, not the semantic source of truth. |
